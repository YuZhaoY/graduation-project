...
@Service
@RocketMQMessageListener(topic = "SALE_UPDATE", consumerGroup = "CID_SALE_FREEZE_GROUP", selectorExpression = "FREEZE", selectorType = SelectorType.TAG)
public class SaleOrderFreezeListener implements RocketMQListener<String> {
    ...
    public void onMessage(String message) {
        ...
        if(saleDTOS != null && !CollectionUtils.isEmpty(saleDTOS)) {
            ...
            if(Objects.requireNonNull(saleDTO.getFreezeAmount()).compareTo(BigDecimal.valueOf(0.00)) != 0) {
                throw new RuntimeException("采购单已生成付款单，无法更新");
            }
            saleDTO.setFreezeAmount(saleMessage.getAmount());
